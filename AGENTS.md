...
- Clarify every state-changing request with `grill-with-docs`, confirm goal, non-goals, and acceptance evidence, then declare Direct, Goal, or DAG mode with a reason.
- The repository root remains on `main` for production. Normal room branches start from `develop`. Never develop or commit directly on `main` or `develop`; never force-push either branch.
