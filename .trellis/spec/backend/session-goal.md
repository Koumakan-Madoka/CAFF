...
- Conversation metadata field:
  - `conversation.metadata.sessionGoal?: { goalId: string, revision: number, objective: string, status: 'active' | 'paused' | 'complete', createdAt: string, updatedAt: string, completedAt?: string, owner?: { agentId: string, agentName: string }, checklist?: { id: string, text: string, status: 'todo' | 'in_progress' | 'done', createdAt: string, updatedAt: string, completedAt?: string }[] }`
...
- Every normalized Goal has an immutable `goalId` and a positive integer `revision`. New or replacement `set` operations and every `resume` create a new `goal_<uuid>` with revision 1. Pause, complete, owner changes, checklist changes, and automatic error/removed-owner pauses keep the current ID and increment revision. Legacy metadata without these fields receives a deterministic `legacy_goal_<hash>` derived from its original `createdAt + objective` and revision 1, so reads are stable without a backfill.
- Goal Runner messages freeze the claimed Goal's `goalId`, `goalRevision`, and iteration in message metadata. Session reuse consumes those values as trigger evidence; later Goal mutations cannot silently make an already-queued continuation represent a newer revision.
...
- `update-checklist` changes the pending `set` proposal checklist when one exists; otherwise it changes only the current goal checklist. It preserves other pending proposal/ruling metadata and never activates a proposal by itself. An active Goal checklist update migrates a same-epoch runner to the refreshed `goal.updatedAt` key in the same metadata write, so iteration and failure-streak state survive instead of restarting at 1; stale runners remain stale.
...
| set / replace / resume | valid existing or new Goal | set/replace and resume each create a new Goal ID at revision 1; same-Goal mutations cannot retain the old Session-reuse identity |
| pause / complete / set-owner / checklist mutation | existing Goal | Goal ID remains stable and revision increments |
...
| `POST /goal update-checklist` | existing goal and no pending `set` proposal | `200`, Goal ID unchanged, revision incremented, checklist stored, runner iteration/streak preserved, and proposal/ruling metadata retained |
