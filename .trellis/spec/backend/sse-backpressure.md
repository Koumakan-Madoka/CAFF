...
- **Deadline re-arm**: each new blocked episode gets a fresh deadline; drain/close/error/removal clears the timer. Deadline expiry removes the client (`disconnects.drainTimeout++`) and ends the stream.
...
| client never reads (10,000+ × 256KiB) | removed at the 2 MiB budget; peak RSS delta stays tiny; zero writes after removal |
| single frame > 2 MiB | client removed before the frame is written; frame bytes never written |
