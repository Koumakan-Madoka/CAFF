...
Abstracts differences between multiple AI coding CLIs and workflow tools.

Supported platforms include OpenCode, Cursor, iFlow, Codex-style skills,
Kilo, Kiro, Gemini, Antigravity, Qoder, CodeBuddy, and legacy Claude configs.
...
class CLIAdapter:
    ...
    def config_dir_name(self) -> str:
        ...
        Returns:
            Directory name for the selected platform config.
        """
    ...
    def get_config_dir(self, project_root: Path) -> Path:
        ...
        Returns:
            Path to the selected platform config directory.
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Note:
            ...
            Markdown-based CLIs use <config>/commands/trellis/<name>.md
        """
    ...
    def build_run_command(
        ...
        Args:
            ...
            session_id: Optional session ID for platforms that support caller-provided IDs
        ...
        if self.platform == "opencode":
            ...
            # No equivalent privileged skip-permissions flag exists here
    ...
    def build_resume_command(self, session_id: str) -> list[str]:
        ...
        raise ValueError(f"Unsupported platform for resume: {self.platform}")
    ...
    def supports_cli_agents(self) -> bool:
        ...
        True for platforms that can launch agent flows from a terminal command.
        """
    ...
    def requires_agent_definition_file(self) -> bool:
        ...
        Some platforms need explicit agent definition files; others auto-discover.
        """
    ...
    def supports_session_id_on_create(self) -> bool:
        ...
        Some platforms allow caller-provided IDs; others auto-generate or omit them.
        """
...
def get_cli_adapter(platform: str) -> CLIAdapter:
    """Get CLI adapter for the specified platform.

    Args:
        platform: Platform name supported by this adapter.
...
def detect_platform(project_root: Path) -> Platform:
    ...
    4. .cursor directory exists with no competing platform dirs → cursor
    ...
    Returns:
        Detected platform for the current repo shape.
    """
    import os
    ...
    # Check for .cursor directory only when no competing platform config exists
    if (project_root / ".cursor").is_dir() and not _has_other_platform_dir(
        project_root, {".cursor"}
    ):
        return "cursor"
    ...
    # Check for shared skills layout used by codex-style command references
