...
def get_last_tool(log_file: Path, platform: str = "") -> str | None:
    ...
    Supports both flat event logs and nested assistant-message logs.

    Nested assistant-message format:
        ...
        for line in reversed(lines[-100:]):
            try:
                ...
                if platform == "opencode":
                    # Flat event format: {"type": "tool_use", "tool": "bash", ...}
                    if data.get("type") == "tool_use":
                        return data.get("tool")
                else:
                    # Nested assistant-message format: {"type": "assistant", "message": {"content": [...]}}
...
def get_last_message(log_file: Path, max_len: int = 100, platform: str = "") -> str | None:
    ...
    try:
        ...
        for line in reversed(lines[-100:]):
            try:
                ...
                if platform == "opencode":
                    # Flat event format: {"type": "text", "text": "..."}
