...
2. Copies optional files declared in worktree.yaml
3. Sets .current-task in worktree
4. Starts the selected CLI agent in background
5. Registers agent to registry.json

Prerequisites:
    - task.json must exist with 'branch' field
    - agent definition must exist for the selected platform (if that platform requires one)
...
def main() -> int:
    ...
    parser.add_argument(
        "--platform", "-p",
        choices=["cursor", "iflow", "opencode", "codex", "qoder"],
        default=DEFAULT_PLATFORM,
        help="Platform to use (default: opencode)"
    )
    ...
    # =============================================================================
    # Step 3: Prepare and Start CLI Agent
    # =============================================================================
    ...
    # Some CLIs allow caller-provided session IDs; others require log extraction.
    ...
    # Clear inherited nested-session markers so the child CLI can start cleanly.
