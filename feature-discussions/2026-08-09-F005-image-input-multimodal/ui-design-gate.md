...
> **R5 修订（@opus 同步，2026-08-09）**：§1.3 发送条件钉死布尔式 `hasPayload && strip.every(ready)`（存在 pending/rejected 即禁用，P1-3/P1-4）+ rejected 语义与"非法永不进入 strip"矛盾收敛（本地明确非法不入 strip、服务端拒绝进 rejected 卡片，P2-2）；§1.3/§5.6 **request_fingerprint 改由服务端基于实际校验后 bytes 计算**（客户端 digest 仅 hint，P1-1）；§5.6 上传协议改 **可恢复提交协议**（pending reservation → temp → 原子 rename → 最终 DB complete → response 只认 complete，P1-1）；§2.3b/§2.2 补预算与 missing-file 的单一 fail-closed 契约（`IMAGE_PROMPT_BUDGET_EXCEEDED` / `IMAGE_CONTENT_UNAVAILABLE`，P1-4/P1-5）。
> **R6 修订（@opus 同步，2026-08-09）**：§5.6 上传协议补 **fenced lease**（batch 加随机 `lease_token`，CAS 条件 UPDATE 取唯一所有权；同 key 重试命中 pending：同进程 in-flight 共享 promise、lease 有效但非 owner → 结构化 `UPLOAD_IN_PROGRESS` + retryAfter（不启动第二条管线）、lease 过期 → CAS takeover 生成新 token；最终 complete 带 `WHERE lease_token=<caller>`，stale worker 影响行数 0 被拒并清理自己临时产物；P1-1）。**会话删除为立即 purge**（P1-2）：同一 transaction 内先删该会话全部 image rows + batch rows 再删 conversation（`image_upload_batches.conversation_id` 明确 FK），DB commit 后 best-effort 删 batch dirs、失败由 reconciliation 清孤儿——UI 无新增交互，会话删除时图片随会话一起消失（前端删除会话后无需处理 strip 遗留，自然随会话清除）。
> **R7 契约加固（@opus 同步，2026-08-09）**：对齐 spec/README（command-center 平行 review `001343`，不重开已 PASS 的 Gate）：§1.2/§5.6 上传**整批消费**——消息须引用 batch 全部 staged children，部分 attach → 400 `IMAGE_PARTIAL_BATCH_ATTACH_REJECTED`（前端 strip 全量引用，不存在部分成功卡片）；`rejected` 只由**确定性校验失败**进入（服务端校验失败 → 该 batch 全部卡片标 rejected + 原因，与本地 UX 预检拦截并存）；上传进行中 `UPLOAD_IN_PROGRESS`/同进程重试均不产生第二张卡片；§3 证据矩阵补 `UPLOAD_IN_PROGRESS` 重试无重复卡片行（P2-2）。
...
- **图片项状态机（R4 P2-1 定案）**：每张图状态为 `pending_validation → ready | rejected`：
  ...
  - `rejected`：上传失败/校验失败 → 卡片标记错误 + 人话原因 + 可移除；**保留可见错误卡片**（R5 P2-2：不自动移除），operator **必须先移除 rejected 项才能发送**（存在 rejected 项即禁用发送，见 §1.3 发送条件）——本地明确非法不入 strip 与"服务端拒绝进 rejected 卡片"并存：前者指本地可判定的格式/尺寸/张数超限（UX-only 预检拦截），后者指服务端最终权威校验失败（进 strip 后标记 rejected）。
...
- **上传重试与幂等（P1-2 + R4 P1-1 + R5 P1-1 修订）**：`clientRequestId` 在**首次上传尝试前**生成（选择第一张图进 strip 时）并随**冻结的 batch payload** 绑定；**`request_fingerprint` 由服务端基于实际校验后的 bytes 计算**（客户端本地 digest 最多是 hint，服务端必须重算/核对，不同 payload 不能伪造同 fingerprint，R5 P1-1）；上传响应丢失 / network-unknown 时**保留 request id + 已返回的 imageIds**，重试用同一 `clientRequestId` 走 canonical batch 恢复（服务端返回既有 `{ images }`），不重复上传。**若 batch 内容已变更（增删图/改顺序/换文件）→ 换新 `clientRequestId` 重新上传**（同 key 异 payload 服务端返回 409 `UPLOAD_IDEMPOTENCY_CONFLICT`）。
- **发送启用条件（R4 P2-1 + R5 P1-3 钉死布尔式）**：
  ```
  hasPayload = content.trim().length > 0 || strip.length > 0
  canSend    = hasPayload && strip.every(item => item.status === 'ready')
  ```
  存在 `pending_validation`/`rejected` 项时 `canSend` 为 false（未拿到 imageId 不得发送；rejected 项保留时必须先移除才可发送）；strip 为空时纯文本照常发送。**四象限测试矩阵**：空文本+空 strip（不可发）、有文本+空 strip（可发）、空文本+strip 全 ready（可发，image-only）、有文本+strip 含 pending/rejected（不可发）。
- **提示双通道**：
...
- **本地明确非法不入 strip，服务端拒绝进 rejected（R5 P2-2 收敛）**：本地可判定的非法（格式不在白名单、尺寸超限、animated GIF、第 6 张）在**进入 strip 前**被前端预检拦截（UX-only，人话提示，不进 strip）；服务端最终权威校验失败的文件**进入 strip 并标记 `rejected`**（卡片可见、可移除，移除后才能发送）。发送按钮不被合法选择阻塞（可发纯文本），但 strip 含 `pending_validation`/`rejected` 项时**不可发送**（`canSend` 布尔式，见上）。
- **image-only 发送**：strip 非空 + 文本为空时可发送（P1-3）；`canSend` 布尔式已涵盖（空文本 + strip 全 ready → true）。
...
- **预算/缺文件同样走 per-invocation block（R5 P1-4）**：图片 prompt 预算超限 → `IMAGE_PROMPT_BUDGET_EXCEEDED`；attached 行 `integrity_status='missing_file'` 或运行期读文件失败 → `IMAGE_CONTENT_UNAVAILABLE`。三者共用同一 UI carrier（trace pill / inline note）与同一 assistant `status='failed'` 失败路径（invocationBlocks + failedReplies + startRun 未调用 + queue 继续）。
- **UI carrier（R3 定案 + R4 文案修正 + R5 扩展）**：该 block 渲染为消息上的 **trace pill / inline note**（复用现有 trace pill 语言与色调，文案以服务端 reason 为准；**不得写"已跳过图片上下文"**——语义上不是剥图继续），**图片仍完整显示在用户消息上**；不改变消息本体、不删除图片、时间线不出现死消息。queue 继续，其他 agent 正常回复。
...
| AC-C1 `UPLOAD_IN_PROGRESS` / 整批消费（R7 P1-3/P2-2 补行） | 同 key 重试命中非 owner 持有 lease → 提示"上传仍在进行，请稍候"（retryAfter 后重试），**不产生第二张卡片**；部分 attach → 400 不产生部分成功态 | UI 单测（UPLOAD_IN_PROGRESS 分支 + retry 无重复 strip 项）+ verifier | 进行中提示 + 重试后仍单卡片截图 | 同左 |
...
| AC-C1 发送条件四象限（R5 P1-3 补行） | `hasPayload && strip.every(ready)`：空文本+空 strip（不可发）、有文本+空 strip（可发）、空文本+strip 全 ready（可发）、有文本+strip 含 pending/rejected（不可发） | UI 单测（四象限矩阵）| 每象限按钮态截图 | 同左 |
...
6. **上传响应契约（R2 P2-1 + R3 P1-2 + R4 P1-1/P1-2 + R5 P1-1 + R6 P1-1 + R7 修订）**：`POST /api/conversations/:id/images` 为 **batch 上传**，返回 **`{ images: [{ imageId }] }`**（有序，与请求 file 顺序一致），不返回持久 URL；预览全部用 `URL.createObjectURL` 本地 objectURL，发送成功后 revoke。**上传幂等**：请求携带 `clientRequestId`（**首次上传前生成**，选择第一张图时）；**`request_fingerprint` 由服务端对实际校验后的 multipart bytes 计算**（客户端本地 digest 仅 hint，服务端重算/核对，R5 P1-1），丢响应/network-unknown 重试同 key+同 fingerprint 返回 canonical batch，前端保留 request id + 已收 imageIds；**batch payload 变更 → 换新 key**（同 key 异 payload → 409 `UPLOAD_IDEMPOTENCY_CONFLICT`）。**可恢复提交协议 + fenced lease + filesystem fencing（R5 P1-1 + R6 P1-1 + R7 P1-1）**：服务端先持久化 pending reservation（随机 `lease_token` + `lease_expires_at`）→ 全量预检写 **token/attempt 隔离临时目录**（`.tmp/<batch_id>/<lease_token>/`，不同 lease 写各自 attempt，stale worker 只清自己）→ 同文件系统原子 rename batch 目录到 final → 最终 SQLite transaction 校验 expected_count/children/final paths 后插 image rows + 条件 UPDATE 置 `complete`（**带 `WHERE status='pending' AND lease_token=<caller>`**；影响 0 行 → 整事务回滚）→ **response 只认 complete**（batch + row_count + slots + files 全满足才返回 `{ images }`）；同 key 命中 pending：同进程 in-flight 共享 promise、lease 有效但非 owner → `UPLOAD_IN_PROGRESS` + retryAfter（不得 resume）、过期 → CAS takeover 后 reconcile/重跑；**不存在"部分成功"响应分支**。上传进行中用户换 key（同批次二次上传）由前端按 `UPLOAD_IN_PROGRESS` 提示"上传仍在进行，请稍候"，不产生第二张条目标记。**R7 整批消费 + rejected 执行路径（P1-3/P2-1）**：消息发送引用 batch 的**全部** staged children（整批 attach，部分 → 400 `IMAGE_PARTIAL_BATCH_ATTACH_REJECTED`，前端 strip 全量引用，不产生部分成功态）；服务端**确定性校验失败**（magic-byte/像素/大小/张数/文件名）→ batch `rejected`（终态），该 batch 全部卡片标 `rejected` + 原因展示（可移除，移除后才能发送）；存储/DB/crash 类失败保持 `pending`，前端按网络重试语义处理。
