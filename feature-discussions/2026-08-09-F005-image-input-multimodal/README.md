...
- **batch 提交协议（R5 P1-1：crash-consistent 可恢复协议，替代 R4 "单次 DB commit all-or-nothing"）**：pending reservation（服务端算 fingerprint + `lease_expires_at`）→ 全量预检写临时目录 → 同文件系统原子 rename batch 目录到 final → **最终 SQLite transaction**（校验 expected_count/children/final paths 后插 image rows + 置 `complete` + `completed_at`）→ response 只认 complete。pending 命中 lease 有效 resume/reconcile、过期回收重跑；complete ⟺ completed_at NOT NULL ⟺ child count = expected_count。补 crash 点测试：pending reservation 后、temp 写入中、rename 后/DB complete 前、complete 后/response 前。
- **生命周期以 `image_upload_batches` + `image_uploads` registry 表为持久化真相源（P1-2/P1-3 + R4 + R5）**：图级状态机 `staged` → `attached` → `recycled`（R5 P1-2 弃 `broken` 状态——文件缺失统一以 `integrity_status='missing_file'` 表达）+ `integrity_status`（R4 P1-5，attached 缺文件保留消息关联）；唯一约束 `UNIQUE(batch_id, slot)`（R5 P1-2 修正 R4 无效约束，`conversation_id`/`client_request_id` 经 batch FK 提供）；`attached_message_id` FK `ON DELETE RESTRICT`；**attach 单事务**（message INSERT + image 行条件 UPDATE 同一 SQLite transaction）；两阶段幂等矩阵（上传同 key+同 fingerprint canonical batch / 同 key 异 payload 409 / 消息同 key canonical result / 不同 key 引用已 attached 明确拒绝）；`staged` 超 TTL（24h）由 GC 清理；消息删除后 attached → recycled（选定一种，不做转 staged 复用）；启动时 DB/文件 reconciliation；child GC 后 batch 行同步清理（R5 P2-1）。
