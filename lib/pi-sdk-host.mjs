...
export async function runAgentRuntime(runtime, prompt, send, options = {}) {
  ...
  const requestRecovery = (input = {}) => {
    ...
    recoveryChain = recoveryChain.then(async () => {
      ...
      // the assistant error messages emitted by our own abort so the runtime can
      // drop exactly those artifacts when recovery succeeds; genuine provider
      // errors keep their identity and still fail the run.
      ...
      try {
        if (session && typeof session.subscribe === 'function') {
          unsubscribeCapture = session.subscribe((event) => {
            ...
            if (message && message.role === 'assistant' && message.stopReason === 'error' && message.errorMessage) {
