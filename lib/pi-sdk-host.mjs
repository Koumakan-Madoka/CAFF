...
import { installProxyOnlyRoutingDispatcher, isProxyOnlyRoutingDispatcherInstalled } from './proxy-routing.mjs';
...
let envProxyDispatcherRepaired = false;

function resolveSdkEntryUrl() {
  try {
    return import.meta.resolve(SDK_SPECIFIER);
  } catch {
...
/**
 * npm undici >= 8 replaces the legacy global dispatcher symbol
 * (undici.globalDispatcher.1) that Node's built-in fetch reads as soon as its
 * module graph loads, which discards the EnvHttpProxyAgent that
 * NODE_USE_ENV_PROXY installed at process start. Model-call fetches then
 * bypass HTTP(S)_PROXY and connect directly. Re-install an EnvHttpProxyAgent
 * from the same undici copy the SDK resolved so proxy routing is restored.
 */
export function repairEnvProxyGlobalDispatcher() {
  if (isProxyOnlyRoutingDispatcherInstalled()) {
    // The whitelist routing dispatcher from proxy-routing.mjs owns the global
    // dispatcher, so this full env-proxy repair defers. It only defers on a
    // successful install — if the routing install failed, this repair still
    // gets its chance below.
    return;
  }
  ...
  if (envProxyDispatcherRepaired) {
    return;
  }
  const legacySymbol = Symbol.for('undici.globalDispatcher.1');
  const current = globalThis[legacySymbol];
  if (!current || current.constructor?.name === 'EnvHttpProxyAgent') {
    return;
  }
  envProxyDispatcherRepaired = true;
  try {
    const sdkRequire = createRequire(resolveSdkEntryUrl());
    const undici = sdkRequire('undici');
    if (
    ...
    // Best effort: if the undici copy cannot be resolved the dispatcher stays
    // as-is and outbound requests follow whatever routing is installed.
  }
}

// DD-2 of docs/engineering/skills/skill-scoping.md: retire the user-scope
// skill roots (agentDir/skills, i.e. .pi-sandbox/skills, plus the empty home
// roots as defense in depth) as cross-project auto-injection sources. The
// filter is by resolved source path only — never by skill name — so it cannot
// drop project-layer skills discovered from the session cwd chain, nor
// contract-layer skills contributed through additionalSkillPaths.
const RETIRED_HOME_SKILL_ROOTS = ['.agents/skills', '.pi/agent/skills'];

export function createRetiredUserScopeSkillFilter(agentDir) {
  const retiredRoots = [
    path.join(path.resolve(agentDir), 'skills'),
    ...RETIRED_HOME_SKILL_ROOTS.map((relative) =>
      path.join(os.homedir(), ...relative.split('/'))),
  ];
  ...
  return (skillsResult) => {
    const result = skillsResult && typeof skillsResult === 'object'
      ? skillsResult
      : { skills: [], diagnostics: [] };
    const skills = Array.isArray(result.skills) ? result.skills : [];
    const retained = skills.filter((skill) => {
      const filePath = String(skill && skill.filePath || '').trim();
      ...
      return !retiredRoots.some((root) => isUnderRoot(resolved, root));
    });

    return retained.length === skills.length ? result : { ...result, skills: retained };
  };
}
...
export function normalizeStartCommand(command) {
  ...
  return {
    ...
    config: {
      ...
      additionalSkillPaths: normalizeExtensionPaths(input.additionalSkillPaths),
      retireUserScopeSkills: Boolean(input.retireUserScopeSkills),
  ...
  const createRuntime = async ({ cwd, agentDir, sessionManager, sessionStartEvent }) => {
    const services = await sdk.createAgentSessionServices({
      ...
      resourceLoaderOptions: {
        ...
        ...(config.retireUserScopeSkills
          ? { skillsOverride: createRetiredUserScopeSkillFilter(config.agentDir) }
          : {}),
      },
    });
...
// pi records an aborted in-flight assistant invocation as an assistant
// message with stopReason "error" and the AbortError message ("This operation
// was aborted" / "The operation was aborted"). Only these exact signatures
// are confirmed abort artifacts; any other assistant error keeps its identity
// as a genuine failure.
const ABORT_ERROR_MESSAGE_PATTERN = /\boperation was aborted\b/i;

function isAbortArtifactErrorMessage(errorMessage) {
  return typeof errorMessage === 'string' && ABORT_ERROR_MESSAGE_PATTERN.test(errorMessage);
}
...
export async function runAgentRuntime(runtime, prompt, send, options = {}) {
  ...
  const requestRecovery = (input = {}) => {
    ...
    recoveryChain = recoveryChain.then(async () => {
      // The recovery abort itself produces assistant error entries (for example
      // an aborted invocation recorded as "This operation was aborted"). Capture
      // only the assistant error messages that carry the abort signature so the
      // runtime can drop exactly those artifacts when recovery succeeds; any
      // other error recorded inside the window is preserved as a genuine
      // provider failure and still fails the run.
      const abortedAssistantMessages = [];
      let unsubscribeCapture = null;

      try {
        if (session && typeof session.subscribe === 'function') {
          unsubscribeCapture = session.subscribe((event) => {
            ...
            if (
              ...
              && isAbortArtifactErrorMessage(message.errorMessage)
            ) {
              abortedAssistantMessages.push(message);
            }
          });
        }
        ...
        await onRecoveryStarted({ ...request, abortedAssistantMessages });
        await recoveryPromptPromise;
        await session.waitForIdle();
      } catch (error) {
        if (!isStopping()) {
...
export function startProcessHost(options = {}) {
  ...
  // Best-effort capture of the harness-assembled system prompt (the exact
  // text the model receives, including pi-injected layers such as the skills
  // list, tool definitions, and project context). Reported once when the
  // runtime is ready and again after the turn so the parent can record the
  // effective prompt in the context snapshot. Failures never break the run.
  async function reportSystemPrompt(phase) {
    if (stopping) {
      return;
    }

    try {
      // The SDK exposes the final assembled prompt at
      // session.agent.state.systemPrompt (see pi-coding-agent docs/sdk.md).
      const agentState = runtime && runtime.session && runtime.session.agent && runtime.session.agent.state;
      const systemPrompt =
        agentState && typeof agentState.systemPrompt === 'string' ? agentState.systemPrompt : '';

      if (systemPrompt) {
        await send({ type: 'system_prompt', phase, systemPrompt });
      }
    } catch (error) {
      writeStderr(`system prompt report failed (${phase}): ${error?.stack || error?.message || String(error)}`);
    }
  }
  ...
  async function start(command) {
    try {
      ...
      const proxyRouting = installProxyOnlyRoutingDispatcher();
      if (!proxyRouting.installed) {
        if (proxyRouting.reason === 'no_proxy_url') {
          writeStderr('PROXY_ONLY_HOSTS is set but HTTP(S)_PROXY is not configured; outbound requests stay direct');
        } else if (proxyRouting.reason === 'invalid_proxy_url') {
          writeStderr('PROXY_ONLY_HOSTS is set but HTTP(S)_PROXY is not a valid http(s) proxy URL; outbound requests stay direct');
        } else if (proxyRouting.reason === 'undici_unavailable') {
          writeStderr('PROXY_ONLY_HOSTS is set but the SDK undici copy could not be resolved; whitelist proxy routing was not installed');
        }
        repairEnvProxyGlobalDispatcher();
      }
      ...
      await reportSystemPrompt('ready');
