...
const {
  augmentSyntheticSeed,
  generateSyntheticSeed,
} = require('./p2c-expand/synthetic-seed');
...
const BUDGET = {
  ...
  heapDeltaMiB: 96,
  rssDeltaMiB: 160,
  retainedHeapDeltaMiB: 32,
  databaseDeltaMiB: 32,
};

function parseArgs(argv) {
  const options = {
    outDir: path.join('.tmp', 'p2c-expand-gate'),
    regenSeed: false,
    baseSeedDir: '',
  };
...
async function runChild(workDir, manifestPath) {
  ...
  const child = spawn(process.execPath, [
    '--expose-gc',
    path.join(__dirname, 'p2c-expand', 'gate-child.js'),
    '--work-dir', workDir,
...
function evaluate(run) {
  ...
  const databaseDeltaMiB = metrics.databaseDeltaBytes / MIB;
  ...
  checks.push({
    name: `database delta ${databaseDeltaMiB.toFixed(1)}MiB <= ${BUDGET.databaseDeltaMiB}MiB`,
    pass: databaseDeltaMiB >= 0 && databaseDeltaMiB <= BUDGET.databaseDeltaMiB,
  });
...
async function main() {
  ...
  if (!manifest) {
    if (options.baseSeedDir) {
      ...
      manifest = augmentSyntheticSeed({ outputDir: seedDir, manifest });
    } else {
      process.stdout.write('gate: generating P2C-Expand production-shape seed; this can take several minutes...\n');
      manifest = generateSyntheticSeed({ outputDir: seedDir });
    }
  } else if (!manifest.p2cExpand) {
    manifest = augmentSyntheticSeed({ outputDir: seedDir, manifest });
  }
  ...
  if (manifest.p2cExpand.integrityCheck !== 'ok' || manifest.p2cExpand.foreignKeyViolations !== 0) {
    throw new Error(`seed integrity=${manifest.p2cExpand.integrityCheck}, foreign keys=${manifest.p2cExpand.foreignKeyViolations}`);
  }
  if (manifest.p2cExpand.detailCounts.contextSnapshots !== 0 || manifest.p2cExpand.detailCounts.modelUsage !== 0) {
    throw new Error('seed must remain old-only without detail backfill');
  }
  ...
  const workDir = path.join(outDir, 'run-expand');
  ...
  const report = {
    gate: 'p2c-expand-message-details',
    startedAt,
