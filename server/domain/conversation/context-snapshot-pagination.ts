import {
  DEFAULT_CONTEXT_SNAPSHOT_PAGE_LIMIT,
  MAX_CONTEXT_SNAPSHOT_PAGE_LIMIT,
} from '../../../lib/message-detail-contract';
...
function parsePageLimit(value: string | null) {
  if (value === null || value === '') {
    return DEFAULT_CONTEXT_SNAPSHOT_PAGE_LIMIT;
  }
  if (!/^\d+$/.test(value)) {
    throw createHttpError(400, 'Context snapshot page limit must be a whole number');
  }

  const limit = Number(value);
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > MAX_CONTEXT_SNAPSHOT_PAGE_LIMIT) {
    throw createHttpError(
      400,
      `Context snapshot page limit must be between 1 and ${MAX_CONTEXT_SNAPSHOT_PAGE_LIMIT}`
    );
  }
  return limit;
}
...
function decodeCursor(value: string | null, conversationId: string) {
  ...
  if (value.length > MAX_CURSOR_LENGTH || !BASE64URL_PATTERN.test(value)) {
    throw createHttpError(400, 'Invalid context snapshot page cursor');
  }
...
export function buildContextSnapshotPage(store: any, conversationId: string, searchParams: URLSearchParams) {
  const limit = parsePageLimit(searchParams.get('limit'));
  const before = decodeCursor(searchParams.get('before'), conversationId);
  const page = store.listContextSnapshotPage(conversationId, { limit, before });
