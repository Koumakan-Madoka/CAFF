...
function buildAgentToolInstructions(agentToolRelativePath: string) {
  ...
  return [
    ...
    '- Room workspace: use the model-visible `room_workspace_preview` tool with no arguments to inspect the server-derived branch/worktree; only call `room_workspace_bind` with `{ "confirm": true }` after the user explicitly confirms creation. Never invent or pass conversation IDs, branches, paths, repositories, or base refs.',
  ].join('\n');
}
