...
class RunTaskRepository {
  ...
  appendEvent(taskId, eventType, eventJson, createdAt) {
    this.insertEventStatement.run(taskId, eventType, eventJson, createdAt);
  }

  addArtifact(taskId, payload) {
  ...
  listByParent(parentTaskId = null) {
...
function createRunTaskRepository(db) {
  return new RunTaskRepository(db);
}

module.exports = {
  RunTaskRepository,
  createRunTaskRepository,
};
