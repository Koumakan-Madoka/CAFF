...
- `busy → reusable`（干净空回复）：聊天交付失败不等于 session 不可续接。仅当运行时证明本轮由模型终止消息正常结束（`completionStopReason` 非空，即 `isTerminalAssistantMessage` 通过：stopReason 非 error/aborted/tool_use/pause_turn 且末条消息无未决 tool_use）且退出时 `openToolCallCount === 0`，空回复失败路径才用与成功路径相同的受守卫 upsert 登记复用；聊天消息仍标 failed（分类 `unknown/unclassified_invocation_error`，不触发成功回调、不伪造回复、不自动重试）。`completionStopReason` 只能由模型终止消息产生，caller 驱动的 `handle.complete()`（bridge 自动完成、delegation yield）恒为 null，故不满足门禁。证据缺失或矛盾（无 completionStopReason、openToolCallCount 非 0、resultPromise 拒绝如超时/取消/模型错误/进程退出）一律沿用原有 poison/fresh 保护。登记复用成功路径同款游标推进（冻结基线 + 本轮 failed assistant），usage 取自本轮 `usageCalls`；登记失败只记日志，遗留 busy 行交给既有 busy-stale 清扫。
