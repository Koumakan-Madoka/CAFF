...
- `busy → reusable`（干净空回复）：聊天交付失败不等于 session 不可续接。仅当运行时证明本轮由模型终止消息正常结束（`completionStopReason` 非空，即 `isTerminalAssistantMessage` 通过：stopReason 非 error/aborted/tool_use/pause_turn 且末条消息无未决 tool_use）、退出时 `openToolCallCount === 0` 且父运行时核验 session 产物落盘（`completionPersisted === true`），空回复失败路径才用与成功路径相同的受守卫 upsert 登记复用；聊天消息仍标 failed（分类 `unknown/unclassified_invocation_error`，不触发成功回调、不伪造回复、不自动重试）。`completionStopReason` 只能由模型终止消息产生，caller 驱动的 `handle.complete()`（bridge 自动完成、delegation yield）恒为 null，故不满足门禁。`completionPersisted` 由父运行时在子进程 close 后亲自核验（`lib/pi-runtime.ts#verifyCompletionPersisted`）：pi 在 `message_end` 先向订阅者发事件、后才 `appendMessage` 落盘，且 `expected_completion` 分支会把真实退出码归一为 0，因此事件时序与退出码都不能证明落盘；核验读取 session 文件尾部（上限 1 MiB，截断首行残片），从尾向前定位最后一条 assistant 消息条目，要求其 `getAssistantMessageKey` 键与触发本次 `expected_completion` 的终止消息一致且归一化 stopReason 相同。文件缺失/为空、尾部为上一轮旧消息、尾行截断（写盘中途被杀）、终止消息之后出现其他 assistant 条目（如 abort 残留）、尾部存在不可解析行，一律判 false（"未证明落盘"，不等于证明丢失）；caller 驱动 complete 的 key/stopReason 恒为 null，核验自然为 false。证据缺失或矛盾（无 completionStopReason、openToolCallCount 非 0、completionPersisted 非 true、resultPromise 拒绝如超时/取消/模型错误/进程退出）一律沿用原有 poison/fresh 保护。登记复用成功路径同款游标推进（冻结基线 + 本轮 failed assistant），usage 取自本轮 `usageCalls`；登记失败只记日志，遗留 busy 行交给既有 busy-stale 清扫。
...
- `tests/runtime/session-reuse-ab.test.js`：flag OFF 字节级不变、普通复用全链路、Goal Runner 同 ID/revision 且 `<50%` resume、revision 变化与 `=50%` fresh、人工触发不收紧、provider 已知 Goal 版本写回/继承、claim 先于 startRun、完整公共 cursor 指纹和私聊 mailbox cursor 下传、executor 级私聊 mailbox cursor 续跑与授权新消息去重、delta-only prompt、resume snapshot 的唯一 `session_delta` 与实际 prompt 逐字一致且不含游标前历史、授权私聊新消息进入 delta 且旧消息不重复、retained prefix 仅为引用、判定后/claim 前编辑触发 poison、运行中新增消息留给下一轮、private-only 与当前 turn 未完成 assistant 采用 fresh 可见性投影、已被中间游标越过的 private handoff 仍以清洗后 anchor 必达、`busy_stale` 审计、per-agent 关闭、编辑即 poison + 自愈；干净空回复（completionStopReason + openToolCallCount=0 + completionPersisted=true）在 fresh/resume 两条路径上消息仍 failed 但登记复用且下一轮实际 resume 同一 session，证据缺失、openToolCallCount>0 或 completionPersisted 缺失/false 时空回复保持 poison/fresh 保护，空回复期间到达的公共/授权私聊消息留在游标之后且下一轮 delta 可见，登记抛异常不崩且 resume 行保持 busy 待 busy-stale 清扫，受守卫 upsert 不覆盖其他 run 持有的 busy claim，progress timeout 拒绝仍 poison 既有 claim；另经真实 routing executor 以最近 24 条 prompt 投影运行超过 24 条的会话，验证 fresh 建立完整游标且下一轮实际 resume。
- `tests/runtime/pi-runtime.test.js`：`completionPersisted` 父侧核验——终止消息未落盘且 exit 7 被归一化仍为 false、尾部精确匹配终止消息为 true、尾部为旧轮消息/尾行截断/终止消息后追加 abort 残留均为 false、caller 驱动 complete() 恒为 false；归一化、超时、恢复与错误分类语义不变。
