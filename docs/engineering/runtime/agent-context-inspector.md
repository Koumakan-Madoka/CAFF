...
- Session lifecycle and provider cache are separate dimensions. The reason-suffixed fresh label `新建 Session · <理由短标签>` (reason label from the frontend `chat.sessionReuseReasonLabel` mapping of the closed-set `sessionReuseReason` code; unknown codes fall back to the raw code, `reused` shows no reason) is rendered only in the message-card observability timeline first step, the Trace summary Session entry, and the context-snapshot metadata row. The Trace "models & tools" event list and the Trace Markdown export still show the bare `新建 Session` / `复用旧 Session` labels from the server-side `sessionProjection.label` (known gap, tracked as a follow-up nit; lifting the reason into the server label must remove the client-side suffix to avoid duplication). Cache read, cache hit, and provider miss remain independent call evidence. The persisted `coldStart*` compatibility fields remain unchanged.
...
| First model call in fresh/resume | Message-card timeline first step, Trace summary Session entry, and snapshot metadata row show `新建 Session · <理由短标签>` / `复用旧 Session`; the Trace event list and Markdown export show the bare label (known gap); render provider cache result separately. |
...
- UI: jsdom renders resume as “复用旧 Session（仅追加增量）”, labels totals as this-turn appended values, shows retained prefix/cache evidence, shows the fresh-session refusal reason as a `未复用原因` meta row and appends it to the trace-summary Session item, never uses the old “系统提示词 tokens 总数” or visible “冷启动” labels, and does not mislabel schema v1 reused snapshots as fresh.
