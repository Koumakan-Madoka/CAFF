...
- Session lifecycle and provider cache are separate dimensions. The first model call is labeled `新建 Session · <理由短标签>` for `fresh` (reason label from the frontend `chat.sessionReuseReasonLabel` mapping of the closed-set `sessionReuseReason` code; unknown codes fall back to the raw code, `reused` shows no reason) or `复用旧 Session` for `resume`; cache read, cache hit, and provider miss remain independent call evidence. The persisted `coldStart*` compatibility fields remain unchanged.
...
| First model call in fresh/resume | Show `新建 Session · <理由短标签>` / `复用旧 Session`; render provider cache result separately. |
