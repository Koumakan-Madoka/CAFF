...
- Shared catalog options carry `runtimeResolvable`. Unresolvable synthetic env/hardcoded defaults are omitted; explicit models.json entries remain diagnostic, and shared selectors require `runtimeResolvable=true` to enable a choice. A missing flag means unverified, not resolved. For legacy injected catalogs, the Recovery response normalizes missing flags through `getResolvedModel` when available, using positively verified matching model identity and thinking capabilities; absent, unverified or failed resolution never defaults to ready. Explicit `false` remains blocked. Legacy shared-picker producers must supply positive resolution evidence; role default resolution itself is unchanged. An explicit Recovery model may resolve against the local registry without expanding the shared picker; provider-prefixed model IDs retain the existing exact-then-unprefixed resolution behavior. Only the Recovery response includes that resolved selection for editing.
