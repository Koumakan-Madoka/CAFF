...
- Endpoint diagnostics (`server/domain/models/endpoint-diagnostics.ts`, browser mirror `public/shared/endpoint-diagnostics.js`) are advisory only and come in two tiers. **Verified combinations** (currently only Kimi For Coding on `api.kimi.com`, both directions: `anthropic-messages ↔ https://api.kimi.com/coding`, `openai-completions ↔ https://api.kimi.com/coding/v1`) produce a `mismatch` diagnostic with an explicit suggestion — and only when the endpoint matches the official form exactly (scheme, host, default port, no credentials/query/fragment, exact path); a custom port, scheme, query, or credential is a different endpoint the evidence says nothing about and never earns a suggestion. Each basis is checkable (pi's vendored registry and the vendored SDKs' path appending are pinned by mock-fetch tests, the openai-compatible URL by the current models.dev `kimi-code-plan-cn` entry). **Unverified hints**: every other well-formed combination earns an `unverified` hint without any suggestion, so silence is reserved for verified matches and can never read as "checked and fine" — `anthropic-messages` ending in `/v1` gets the path-specific double-`/v1` hint; other anthropic/openai-completions/openai-responses endpoints get a neutral hint stating the pinned client path fact; all other dialects (google, custom protocol strings) get a generic hint without path claims. Incomplete or unparseable inputs stay silent. Diagnostics are computed against the *effective* configuration: in the catalog projection a stored provider protocol wins over the catalog dialect (`effectiveDialect`, `dialectConflict`), stored model-level `api`/`baseUrl` overrides are surfaced separately (`modelEndpointOverride`) and keep precedence, and sibling models with overrides are surfaced as `siblingModelOverrides` so the wizard can preview the provider-level impact of a suggestion (a provider base URL is provider-wide). The catalog import UI and the provider editor recompute on every edit and render a warning plus an explicit “apply suggestion” action only when a verified suggestion exists, together with the sibling impact listing; the POST import response reports the post-import effective diagnostics (`endpointDiagnostic`, `modelEndpointDiagnostic`, `siblingEndpointDiagnostics`) and the import wizard renders them in a post-import advisory panel that keeps the page until the user explicitly finishes. The advisory state is a **read-only result card** showing the persisted combination from the import response (never the stale catalog projection — the editable form is gone because the main button has become 完成 and would not save). Completion is an explicit handback: the 完成 and close buttons fire `onImported` (refresh + pane handover) exactly once per pending import; in-wizard navigation (browsing another model, collapsing a provider, refreshing/reopening the catalog) keeps the handback pending without firing the callback or letting the parent reclaim the pane, so a delayed provider refresh can never disable or overwrite the page the user is browsing; a failed refresh keeps the advisory state and offers an explicit retry instead of a dead disabled button. Abnormal exits are covered too: leaving from a failed-catalog-reload or unavailable-snapshot page (where no catalog index is loaded) still flushes the pending handback instead of crashing, and leaving while an import POST is still in flight waits for it — the handback refresh includes the just-persisted import and a late response can never re-render into the pane the parent owns again; a late import failure keeps the page with the failure visible and the exit retryable. Raw catalog values and existing `models.json` entries are never rewritten silently, and there is no migration for stored configurations.
...
- `tests/runtime/endpoint-diagnostics.test.js`: the verified Kimi For Coding combinations in both directions (including the exact-official-form rule: custom ports, schemes, queries, fragments, and credentials never earn a suggestion), the unverified anthropic `/v1` hint (never a suggestion) for custom gateways, unified neutral hints for every unverified dialect combination, negative cases for incomplete inputs, mock-fetch pins of the vendored Anthropic `/v1/messages`, OpenAI `/chat/completions`, and OpenAI `/responses` path appends, a pin of pi's vendored kimi-coding registry entry, and browser-mirror symmetry across the decision matrix.
- `tests/runtime/catalog-import-ui.test.js` and `tests/runtime/provider-editor-ui.test.js`: diagnostic rendering against the effective dialect, explicit-apply behavior with the sibling impact listing, reverse-direction suggestions after a protocol switch, stale-suggestion guards, dialect-conflict and model-override surfacing (including the api-only override wording), the post-import advisory panel that keeps the page until explicit completion (including a real provider-management integration test), the read-only result card with persisted values, in-wizard navigation keeping the handback pending (including a deferred-fetch management integration test), retryable completion after a failed provider refresh, consecutive imports finishing with a single handback, exits from catalog-reload-failure and unavailable-snapshot pages, and in-flight imports settled before exit (late success and late failure), plus neutral hints for unverified combinations, and custom-gateway no-suggestion guarantees.
