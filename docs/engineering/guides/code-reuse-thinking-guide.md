...
**Symptom**: New platform works partially — some methods return a generic fallback instead of platform-specific values. No error is raised.

**Example** (`cli_adapter.py`):
```python
# BAD: "gemini" falls through to else, returns a generic default
@property
def cli_name(self) -> str:
    if self.platform == "opencode":
        return "opencode"
    else:
        return "default-cli"  # gemini silently gets the wrong fallback!
