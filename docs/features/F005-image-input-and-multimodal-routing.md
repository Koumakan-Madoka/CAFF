...
- **`image_uploads` registry schema（P1-3 定案 + R4 补 batch identity/integrity + R5 补 crash-consistent 提交协议与可执行 schema）**：
  - **批级 `image_upload_batches` 表（R4 P1-2 新增，R5 补 lease/check/postcondition，batch completion truth）**：`batch_id` PK、`conversation_id` NOT NULL、`client_request_id` NOT NULL、`request_fingerprint` NOT NULL、`expected_count` NOT NULL、`status` NOT NULL（`pending | complete | rejected`）、`lease_expires_at`（nullable，R5 P2-1：pending recovery lease）、`rejected_reason`（nullable，R5 P2-1：rejected 终态语义）、`created_at` NOT NULL、`completed_at`（nullable）。**唯一约束** `UNIQUE(conversation_id, client_request_id)`——batch identity 的持久化真相源。**可执行 invariants（R5 P2-1）**：CHECK `1 <= expected_count <= MAX_IMAGES_PER_UPLOAD`；`complete` ⟺ `completed_at IS NOT NULL`（置 complete 时同一 transaction 写 completed_at）；complete 时 child row count = `expected_count` 且 slots 连续 `0..expected_count-1`（后置条件，attach 前校验）；`pending` 行带 `lease_expires_at`（reservation lease），同 key 命中未过期 pending 走 resume/reconcile、过期后同 key 重试重新走全流程；`rejected` 必有 `rejected_reason` 且为终态（TTL GC 清理，不转其他状态）。
...
- **batch 提交协议（R5 P1-1 修正：crash-consistent 可恢复协议，替代 R4 "all-or-nothing 单次 DB commit"）**：`complete` 必须严格等价于"全部最终文件已就绪 + 全部 image rows 完整"。协议顺序：
  1. **pending reservation**：服务端对**实际 multipart bytes** 计算 `request_fingerprint` + 校验 `expected_count`（以实际 multipart 数量为准），持久化 `pending` 行（`(conversation_id, client_request_id)` UNIQUE + fingerprint + expected_count + `lease_expires_at`）；
  ...
  4. **最终 SQLite transaction**：校验 `expected_count`/children（slot 连续 `0..count-1`）/final paths 后插 image rows（`staged`）+ 置 batch `complete` + `completed_at`；
  5. **response 只认 `complete`**（batch 行 + row_count + slots + final files 全满足才返回 `{ images: [{ imageId }] }`）。
  同 key 命中 **pending 行**：lease 未过期 → resume/reconcile（继续原协议，不得当空批次重跑 INSERT——UNIQUE 约束会失败）；lease 过期 → 回收 pending 行后重跑全流程。任何一步失败：pending 行保留（带 lease），临时目录按 lease 回收；**绝不把非 complete 的 batch 当 canonical 返回**。补 crash 点测试：pending reservation 后、temp 写入中、目录 rename 后/DB complete 前、complete 后/response 前。
- **幂等矩阵（P1-2/P1-3 修正 + R4 补 batch identity，两阶段分别幂等）**：
  ...
  | 上传 batch 为 `pending`（中途失败/崩溃） | **无 canonical 可返回**；同 key 重试：lease 未过期 → resume/reconcile 原协议；lease 过期 → 回收 pending 行后重跑全流程（**不得当空批次直接重跑 INSERT**，R5 P1-1） |
...
- [ ] AC-A4: 上传/存储/引用失败时返回结构化错误并展示人话原因，消息写入与图片上传通过幂等流程关联，不产生半个消息或孤儿阻断；**两阶段幂等矩阵（P1-2 R3 + R4 batch identity + R5 服务端 fingerprint）**：上传阶段同 `(conversation_id, client_request_id)` + 同 `request_fingerprint` 重试（响应丢失/network-unknown）返回 canonical complete batch；**同 key 异 payload → 409 `UPLOAD_IDEMPOTENCY_CONFLICT`**；`request_fingerprint` **由服务端对实际校验后的 multipart bytes 计算**（客户端 digest 仅 hint，服务端重算/核对，R5 P1-1）；batch `pending`（中途失败/崩溃）无 canonical、同 key 重试 lease 未过期走 resume/reconcile、过期后回收重跑（**不得当空批次重跑 INSERT**，R5 P1-1）；消息阶段同消息 `clientRequestId` 重试返回 canonical result、不同 key 引用已 attached imageId 拒绝；`client_request_id` 在**首次上传前**由前端生成并跨重试复用；**attach 单事务**：message INSERT + 所有 image 行条件 UPDATE（staged→attached，含 ownership/status/row-count 校验）同一 SQLite transaction，任一行失败整体回滚；**batch 可恢复提交协议（R5 P1-1）**：pending reservation → 全量预检 → temp 目录 → 同文件系统原子 rename batch 目录到 final → 最终 SQLite transaction（校验 expected_count/children/final paths 后插 image rows + complete + completed_at）→ response 只认 complete（batch + row_count + slots + files 全满足）。验证：错误路径测试 + 两阶段幂等矩阵测试（含丢响应重试 + 同 key 异 payload 冲突 + crash 点测试：pending reservation 后/temp 写入中/rename 后 DB complete 前/complete 后 response 前）+ attach 事务原子性测试 + registry reconciliation 测试 + batch 可恢复提交协议测试。
...
3. **上传与发送时序**：**✅ 已定案（Design Gate Review R2 + R3 + R4 + R5，2026-08-09 @opus）**——两阶段（先 upload 拿 opaque imageId → 消息引用 imageId，服务端投影 URL），上传按 **batch 契约**返回 `{ images: [{ imageId }] }`（有序，P1-2）。**图片状态以 `image_upload_batches` + `image_uploads` registry 表为持久化真相源**（P1-2/P1-3 + R4 + R5）：batch 唯一 `UNIQUE(conversation_id, client_request_id)` + `request_fingerprint`（**服务端对实际 bytes 计算**，count + ordered file hashes，同 key 异 payload → 409，R5 P1-1）+ `status` completion truth（complete ⟺ completed_at + child count = expected_count）+ **可恢复提交协议**（pending reservation → temp → 原子 rename → 最终 DB complete → response 只认 complete，R5 P1-1）；图级状态机 `staged → attached → recycled` + `integrity_status`（missing_file，R4 P1-5），唯一 `UNIQUE(batch_id, slot)`（R5 P1-2 修正），`attached_message_id` FK `ON DELETE RESTRICT`，attach 单事务（message INSERT + 条件 UPDATE 同一 transaction），TTL GC、消息删除转 recycled、启动 reconciliation。技术决策，不再升级 operator（D3 收敛）。
