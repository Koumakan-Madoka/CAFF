...
test('goal panel explains an automatic model-failure pause and keeps Resume available', () => {
  ...
  const conversation = {
    ...
    metadata: {
      ...
      sessionGoalRunner: {
        ...
        lastFailureMode: 'provider:quota',
        lastFailureKind: 'provider',
        lastFailureSummary: 'insufficient balance',
        pauseReason: '连续 3 轮同模式模型调用失败（provider:quota），Goal 已自动暂停。',
  ...
  assert.match(details.textContent, /连续失败 3 轮/u);
  assert.match(details.textContent, /同模式 3 轮/u);
...
test('goal panel still explains a legacy fast-failure pause recorded before the dual-counter upgrade', () => {
  const pausedAt = '2026-08-21T00:03:05.000Z';
  const conversation = {
    id: 'conv-error-paused-legacy',
    metadata: {
      sessionGoal: {
        objective: 'Use a healthy provider',
        status: 'paused',
  ...
  assert.match(details.textContent, /模型调用失败自动暂停/u);
  assert.match(details.textContent, /连续失败 3 轮/u);
  assert.doesNotMatch(details.textContent, /同模式/u);
  assert.equal(document.getElementById('session-goal-resume-button').disabled, false);
});
