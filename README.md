...
- **终态不被迟到回复改写**：迟到回复单独标记并保留审计记录，不推翻已结算的执行结果。
...
- Goal Runner 自动续跑额外核对 Goal 身份与版本，并要求上下文占用率低于严格阈值（至多 50%）。
...
| `PI_CHAT_SESSION_REUSE_ENABLED` | Session 复用总开关，设为 `0` 可关闭 |

完整起步配置见 [`.env.example`](.env.example)。凭据留在本地配置中，不要提交到仓库。CAFF 面向可信本地环境；不要未经访问控制与安全评估直接暴露到公网。运行多个实例时，请分别配置端口、数据库与运行目录，不要让多个实例共写同一数据库。
